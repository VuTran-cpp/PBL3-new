...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        {
            db.Roles.AddRange(
                new CafeManagement.Models.Role { Name = "ADMIN" },
                new CafeManagement.Models.Role { Name = "MANAGER" },
                new CafeManagement.Models.Role { Name = "STAFF" });
        ...
        if (!await db.OrderStatuses.AnyAsync())
        {
            db.OrderStatuses.AddRange(
                new CafeManagement.Models.OrderStatus { Name = "PENDING" },
                new CafeManagement.Models.OrderStatus { Name = "COOKING" },
                new CafeManagement.Models.OrderStatus { Name = "SERVED" },
                new CafeManagement.Models.OrderStatus { Name = "COMPLETED" },
                new CafeManagement.Models.OrderStatus { Name = "CANCELLED" });
            await db.SaveChangesAsync();
        }
        ...
        {
            db.Branches.Add(new CafeManagement.Models.Branch
            {
        ...
        // Seed Admin Employee + Account
        if (!await db.Accounts.IgnoreQueryFilters().AnyAsync())
        {
            var branch = await db.Branches.FirstAsync();
            var emp = new CafeManagement.Models.Employee
            {
                BranchId = branch.Id,
            ...
            db.Accounts.Add(new CafeManagement.Models.Account
            {
                EmployeeId = emp.Id,
                RoleId = adminRole.Id,
                Username = "admin",
    ...
    {
        app.Logger.LogWarning(ex, "⚠️ Database seed skipped (tables may already exist via SQL script)");
    }
}
